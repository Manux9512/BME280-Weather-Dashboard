    ...
    if (com_rslt == SUCCESS)
    {
        while (true)
        {
            ...
            if (com_rslt == SUCCESS)
            {
                ...
                {
                    int msg_id;
                    msg_id = esp_mqtt_client_publish(client, MQTT_PUB_TEMP_BME280, temperature, 0, 0, 0);
                    ESP_LOGI(TAG, "Sent temperature publish successful, msg_id=%d", msg_id);

                    msg_id = esp_mqtt_client_publish(client, MQTT_PUB_PRES_BME280, pressure, 0, 0, 0);
                    ESP_LOGI(TAG, "Sent pressure publish successful, msg_id=%d", msg_id);

                     msg_id = esp_mqtt_client_publish(client, MQTT_PUB_HUM_BME280, humidity, 0, 0, 0);
                    ESP_LOGI(TAG, "Sent humidity publish successful, msg_id=%d", msg_id);
